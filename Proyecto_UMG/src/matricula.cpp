...
#include "Sedes.h"
#include "Facultad.h"
#include "Bitacora.h" //implementacion de bitacora
...
int choice;
	...
	cout<<"\t\t\t---------------------------------------"<<endl;
	cout<<"\t\t\t|SISTEMA PAGO DE MATRICULA  -  6300   |"<<endl;
	cout<<"\t\t\t---------------------------------------"<<endl;
...
void matricula::insertar() {
    ...
    if (!fileMat) {
        ...
        cin.getline(matricula.idSede, sizeof(matricula.idSede));

        ifstream archivoSedes("Sedes.dat", ios::binary);
        if (!archivoSedes.is_open()) {
            cerr << "Error: No se pudo abrir el archivo 'Sedes.dat'." << endl;
            return;
        }
        else
        {
            while (archivoSedes.read(reinterpret_cast<char*>(&sede), sizeof(Sede)))
            {
                if (strcmp(sede.id, matricula.idSede) == 0) {
                    sedeEnc = true;
                    break;
                }
            }
            ...
            if (!sedeEnc) {
                cerr << "El id ingresado no coincide con el de ninguna sede" << endl;
                return;
            }
            // Asignar los datos de la sede a la estructura Matricula
            strcpy(matricula.sede, sede.nombre);
            strcpy(matricula.telSede, sede.telefono);
        }

        cout << "\t\t\tIngrese id de la facultad: ";
        char idFac[50];
        cin.getline(idFac, sizeof(idFac));

        ifstream archivoFacultad("facultad.dat", ios::binary);
        if (!archivoFacultad.is_open()) {
            cerr << "Error: No se pudo abrir el archivo 'facultad.dat'." << endl;
            return;
        }
        else
        {
            while (archivoFacultad.read(reinterpret_cast<char*>(&facultad), sizeof(Facultad)))
            {
                int codigoFacultad = atoi(idFac);
                if (facultad.codigo == codigoFacultad) {
                    facEnc = true;
                    break;
                }
            }
            ...
            // Asignar los datos de la facultad a la estructura Matricula
            strcpy(matricula.facultad, facultad.nombre);
        }
    ...
    bool pagoExitoso = pagarEnLinea();
    //pagarEnLinea();
    if(pagoExitoso){
        fileMat.write(reinterpret_cast<char*>(&matricula), sizeof(Matricula));
        cout << "\n\t\t\tLa matricula se ha guardado exitosamente." << endl;
    }
    else {
        cout << "\n\t\t\tEl pago no se ha realizado con exito. La informacion de la matricula no se guardara." << endl;
    }
    fileMat.close();
    system("cls");
...
void matricula::desplegar() {
    ...
    while (fileMat.read(reinterpret_cast<char*>(&matricula), sizeof(Matricula))) {
        ...
        cout << "\t\t\tTelefono de la sede: " << matricula.telSede<< endl;
