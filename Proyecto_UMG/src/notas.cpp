...
// Implementación de la función validarAlumno
bool NotaCrud::validarAlumno(int codigoEstudiante) {
    ifstream archivo("codigos_estudiantes.txt"); // Abrir el archivo de texto
    if (!archivo) { // Verificar si el archivo se abrió correctamente
        cerr << "Error al abrir el archivo de códigos de estudiantes." << endl;
        return false; // Si hay un error al abrir el archivo, devolver falso
    }

    string codigo; // Variable para almacenar cada código de estudiante leído del archivo
    while (getline(archivo, codigo)) { // Leer cada línea del archivo
        // Convertir la cadena a un entero y comparar con el código de estudiante dado
        if (stoi(codigo) == codigoEstudiante) {
            archivo.close(); // Cerrar el archivo
            return true; // Devolver verdadero si se encuentra el código
        }
    }

    archivo.close(); // Cerrar el archivo
    return false; // Devolver falso si el código no se encuentra
}
...
void NotaCrud::CrudNota() {
    ...
    do {
        ...
        switch (opcion) {
            case 1:
<<<<<<< HEAD
                IngresarNota(); // Llama a la funciÃ³n para ingresar una nota

                break;
            case 2:
                ModificarNota(); // Llama a la funciÃ³n para modificar una nota
 // Registra en la bitÃ¡cora la acciÃ³n de modificaciÃ³n de nota
                break;
            case 3:
                BorrarNota(); // Llama a la funciÃ³n para borrar una nota
 // Registra en la bitÃ¡cora la acciÃ³n de eliminaciÃ³n de nota
                break;
            case 4:
                DesplegarNotas(); // Llama a la funciÃ³n para desplegar todas las notas
 // Registra en la bitÃ¡cora la acciÃ³n de lectura de notas
=======
...
void NotaCrud::IngresarNota() {
    ...
    // Solicitar y validar el código del estudiante
    int codigoEstudiante;
    cout << "Ingrese el codigo del estudiante: ";
    cin >> codigoEstudiante;
    cin.ignore(); // Limpiar el buffer de entrada

    if (!validarAlumno(codigoEstudiante)) {
        cout << "El código de estudiante no es válido." << endl;
        system("pause");
        return;
    }

    // El código de estudiante es válido, continuar con la captura de la nota
    nota.codigoEstudiante = codigoEstudiante;
